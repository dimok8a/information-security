...
std::string BigInt::ToString()
{
	...
	for (int i = digits.size() - 1; i >= 0; i--)
		answer += std::to_string((short)digits[i]);
	return answer;
}

std::vector<BigInt> BigInt::splitString(System::String^ message, bool isNumbers)
{
	std::vector<BigInt> allNumbers;
	if (isNumbers)
	{
		...
		// Decompose statement
		while (pos != -1) {
			BigInt newNumber = context.marshal_as<std::string>(message->Substring(0, pos));
			allNumbers.push_back(newNumber);
			message = message->Substring(pos+1, message->Length - pos-1);
			pos = message->IndexOf(" ");
		}
		BigInt newNumber = context.marshal_as<std::string>(message);
		// Add the last one
		allNumbers.push_back(newNumber);
	}
	else
